    ...
    <el-table :data="tabelData" border align="center" size="mini" @cell-click="query">
      <el-table-column v-for="(v, k) in columns" :key="k" :label="v.label" align="center" :class-name="`item${k >= 5 ? ' cur' : ''}`">
        <template slot slot-scope="{ row }">
          <div :class="`wrapper${is_today(row[k]) ? ' cur' : ''}`">
            <span>{{ row[k] }}</span>
            <span
              class="special"
...
export default class AttendanceCalendar extends Vue {
  ...
  columns: {
    label: string;
    prop: string;
  }[] = [{ label: "星期一", prop: "monday" }, { label: "星期二", prop: "tuesday" }, { label: "星期三", prop: "wednesday" }, { label: "星期四", prop: "thursday" }, { label: "星期五", prop: "friday" }, { label: "星期六", prop: "saturday" }, { label: "星期日", prop: "sunday" }];
  index_map = (() => {
    let map = {};
