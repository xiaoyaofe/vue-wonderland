        ...
        <el-col class="item-container" v-for="(item, index) in model.purchaseList" :key="`num_${index}`">
          ...
          <el-row type="flex">
            ...
            <div class="item">
              ...
              <el-col class="value" style="width: 84px">
                <el-input :size="input_size" v-model="model.purchaseList[index].totalBudget" v-required="model.purchaseList[index].totalBudget" type="number" />
              </el-col>
            </div>
...
@Component<Purchase>({
  ...
  computed: {
    purchaseTypeOptions() {
      return [{ value: "0", label: "办公用品" }, { value: "1", label: "日常用品" }, { value: "2", label: "IT设备（键盘、鼠标、显卡、硬盘、显示器等）" }, { value: "3", label: "办公电脑、笔记本电脑" }, { value: "4", label: "测试服务器、渲染服务器" }, { value: "5", label: "测试手机" }, { value: "6", label: "电子产品（平板等）" }, { value: "7", label: "外包服务" }, { value: "8", label: "装修工程" }, { value: "9", label: "其他" }];
    },
    user_position() {
