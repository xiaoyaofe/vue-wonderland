...
import { Vuex, RG_SERVER_RES, RG_SERVER_RES_BASE, Mutation, Getter, Action, Module } from 'types/vuex';
...
export default {
  ...
  } as { [key in keyof Vuex.OA.Mutations]: Mutation<Vuex.OA.State> },
  getters: {
  ...
  } as { [key in keyof Vuex.OA.Getters]: Getter<Vuex.OA.State, any> },
  actions: {
    ...
    user_permission: action(async function ({ $post, $commit }, { UserId }) {
    ...
    */
    ...
    approval_application: action(function ({ $post, $notify }, param) {
      return $post("/approval/application", param, {
        headers: {
    ...
    /** 问卷查询 */
    questionnaire_query: action(function ({ $post, $notify }, data) {
