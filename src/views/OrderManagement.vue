				...
				<el-tab-pane v-for="(value, name) in translation.tabs" :key="name" :label="value" :name="name">
					<template v-if="Version1()">
						<el-row class="mt-16" type="flex" justify="center" align="middle">
							...
							<ApolloMutation
								:mutation="require('src/graphql/OrderManagement.gql').Verify"
								@done="DoneOfVerify"
							>
								<template slot-scope="{ mutate }">
									<el-button @click="QueryTransactionId(mutate)">查询</el-button>
								</template>
							</ApolloMutation>
						</el-row>
						...
						<ApolloQuery
							:query="require('src/graphql/OrderManagement.gql').AppInfos"
							:variables="{options:{name:'appInfos',version: '1'}}"
							:class="RequiredCheck()"
							@result="DoneOfAppInfos"
						>
								...
								<el-row class="mb-16" type="flex" align="middle" justify="center">
									...
									<el-col :sm="10">
										...
										<el-input
											v-else
											v-model="userId"
											:placeholder="(querying && !userId && translation.notification.noUserId) || translation.placeholder.userId"
											clearable
										></el-input>
									</el-col>
								</el-row>
                    ...
                    <el-input v-else v-model="roleId" :placeholder="(querying && !roleId && translation.notification.noRoleId) || translation.placeholder.roleId" clearable></el-input>
								...
								<el-row v-if="appInfos" class="mb-16" type="flex" align="middle" justify="center">
									...
									<el-col :sm="10">
										<el-input
											v-if="verifyData"
											:value="`${verifyData.data.orderInfo.appId} - ${appInfos[verifyData.data.orderInfo.appId] ? appInfos[verifyData.data.orderInfo.appId].appName : ''}`"
											disabled
										></el-input>
										<ApolloMutation
											v-else
											:mutation="require('src/graphql/OrderManagement.gql').AdvChannels"
											@done="DoneOfAppId"
										>
											<template slot-scope="{ mutate }">
												<el-select
                        ...
                        ChangeAppId(mutate)"
												>
													<el-option
														v-for="(item, appId) in appInfos"
														:key="appId"
														:label="`${appId} - ${item.appName}`"
														:value="Number(appId)"
													></el-option>
												</el-select>
											</template>
								...
								<el-row class="mb-16" type="flex" align="middle" justify="center">
									...
									<el-col :sm="10">
										<el-input
											v-if="verifyData"
											:value="`${appInfos[verifyData.data.orderInfo.appId] ? appInfos[verifyData.data.orderInfo.appId].gameZones[verifyData.data.orderInfo.gameZoneId]? appInfos[verifyData.data.orderInfo.appId].gameZones[verifyData.data.orderInfo.gameZoneId].zoneName : verifyData.data.orderInfo.gameZoneId : verifyData.data.orderInfo.gameZoneId}`"
											disabled
										/>
										...
										<el-select
											...
											clearable
											filterable
											:placeholder="translation.placeholder.gameZone"
										>
											<el-option
												v-for="(item, unique) in appInfos[appId].gameZones"
												:key="unique"
												:label="`${item.gameZoneId} - ${item.zoneName}`"
												:value="item.gameZoneId"
											></el-option>
										</el-select>
                  ...
                  <el-col :sm="10">
                    ...
                    <ApolloMutation v-else :mutation="require('src/graphql/OrderManagement.gql').PayChannels" @done="DoneOfAdvChannel">
                      <template slot-scope="{ mutate }">
                        ...
                        <el-select v-else :placeholder="translation.placeholder.advChannel" v-model="advChannel" clearable />
								...
								<el-row class="mb-16" type="flex" align="middle" justify="center">
									...
									<el-col :sm="10">
										...
										<ApolloMutation
											v-else
											:mutation="require('src/graphql/OrderManagement.gql').PayWays"
											@done="DoneOfPayChannel"
										>
											<template slot-scope="{ mutate }">
												<el-select
													v-if="payChannels.length"
													v-model="payChannel"
													clearable
													filterable
													:placeholder="translation.placeholder.payChannel"
													@change="ChangePayChannel(mutate)"
												>
													<el-option
														v-for="item in payChannels"
														:key="`${appId}-${item.channelId}`"
                  ...
                  <el-col :sm="10">
                    ...
                    <ApolloMutation v-else :mutation="require('src/graphql/OrderManagement.gql').Products" @done="DoneOfPayWay">
                      <template slot-scope="{ mutate }">
                        ...
                        <el-select v-else v-model="payWay" :placeholder="translation.placeholder.payWay">
								...
								<el-row class="mb-16" type="flex" align="middle" justify="center">
									...
									<el-col :sm="10">
										<el-select
											v-if="verifyData && verifyData.code === 200"
											v-model="product"
											clearable
											filterable
											:placeholder="translation.placeholder.product"
										>
											<el-option
												v-for="item in verifyData.data.products"
												:key="item.goodsId"
												:label="`${item.goodsPrice}/${item.currency} - ${item.goodsName}`"
												:value="item.goodsId"
											></el-option>
										</el-select>
										<el-select
											v-else-if="verifyData && verifyData.code === 201"
											v-model="product"
											clearable
											filterable
											:placeholder="translation.placeholder.product"
										>
											<el-option
												v-for="item in verifyData.data.productInfoList"
												:key="item.goodsId"
												:label="`${item.goodsPrice}/${item.currency} - ${item.goodsName}`"
												:value="item.goodsId"
											></el-option>
										...
										<el-select v-else v-model="product" :placeholder="translation.placeholder.product"></el-select>
									</el-col>
								</el-row>
							</template>
						</ApolloQuery>
						...
						<el-row class="mt-16 mb-16" type="flex" justify="center">
							<ApolloMutation
								:mutation="require('src/graphql/OrderManagement.gql').Repair"
								@done="DoneOfRepair"
							>
								<template slot-scope="{ mutate }">
									<el-button
										@click="CheckRequired(mutate)"
										style="width: 256px; margin-top: 25px;"
									>{{translation.text.submit}}</el-button>
								</template>
							</ApolloMutation>
						</el-row>
					</template>
					...
					<template v-else>
						...
						<ApolloQuery
							:query="require('src/graphql/OrderManagement.gql').AppInfos"
							:variables="{options:{name:'appInfos',version: '2'}}"
							:class="RequiredCheck()"
							@result="DoneOfAppInfos"
						>
							<template slot-scope="{ result: { data } }">
								...
								<el-row class="mb-16" type="flex" align="middle" justify="center">
									...
									<el-col :sm="10">
										...
										<el-input
											v-else
											v-model="roleId"
											:placeholder="(querying && !roleId && translation.notification.noRoleId) || translation.placeholder.roleId"
											clearable
										></el-input>
									</el-col>
								</el-row>
                      ...
                      ChangeAppId(mutate)"
								...
								<el-row class="mb-16" type="flex" align="middle" justify="center">
									...
									<el-col :sm="10">
										<el-input
											v-if="verifyData"
											:value="`${appInfos[verifyData.data.orderInfo.appId] ? appInfos[verifyData.data.orderInfo.appId].gameZones[verifyData.data.orderInfo.thirdGameZoneId]? appInfos[verifyData.data.orderInfo.appId].gameZones[verifyData.data.orderInfo.thirdGameZoneId].zoneName : verifyData.data.orderInfo.thirdGameZoneId : verifyData.data.orderInfo.thirdGameZoneId}`"
											disabled
										/>
										...
										>
											<el-option
												v-for="(item, unique) in appInfos[appId].gameZones"
												:key="unique"
												:label="`${item.thirdGameZoneId} - ${item.zoneName}`"
												:value="item.thirdGameZoneId"
											></el-option>
										</el-select>
								...
								<el-row class="mb-16" type="flex" align="middle" justify="center">
									...
									<el-col :sm="10">
										...
										<ApolloMutation
											v-else
											:mutation="require('src/graphql/OrderManagement.gql').PayChannels"
											@done="DoneOfAdvChannel"
										>
											<template slot-scope="{ mutate }">
												<el-select
													v-if="appId"
													...
													clearable
													filterable
													:placeholder="translation.placeholder.advChannel"
													@change="
                    ChangeAdvChannel(mutate)"
												>
													<el-option
														v-for="(item, index) in advChannels"
														:key="index"
														:label="`${item.advChannel} - ${item.advChannelName}`"
														:value="item.advChannel"
													></el-option>
												</el-select>
												<el-select
													v-else
								...
								<el-row class="mb-16" type="flex" align="middle" justify="center">
									...
									<el-col :sm="10">
										...
										<ApolloMutation
											v-else
											:mutation="require('src/graphql/OrderManagement.gql').Products"
											@done="DoneOfPayWay"
										>
											<template slot-scope="{ mutate }">
												<el-select
													...
													clearable
													filterable
													:placeholder="translation.placeholder.payWay"
													@change="ChangePayWay(mutate)"
												>
													<el-option
														v-for="item in payWays"
														:key="`${appId}-${item.cardCode}`"
														:label="`${item.cardCode} - ${item.cardCodeName}`"
														:value="item.cardCode"
													></el-option>
												</el-select>
												<el-select v-else v-model="payWay" :placeholder="translation.placeholder.payWay"></el-select>
											</template>
										</ApolloMutation>
									</el-col>
								</el-row>
									...
									<el-col :sm="10">
										...
										>
											<el-option
												v-for="item in verifyData.data.products"
												:key="item.productId"
												:label="`${item.amount}/${item.currency} - ${item.productName}`"
...
<script lang="ts">
	...
	export default class OrderManagement extends Vue {
		...
		DoneOfRepair({ data: { post } }) {
			if (post.code === 200) {
				if (
					!this._data._verifiedId[this.version] ||
					!this._data._transactionId[this.version]
				) {
					this._data._verifiedId[this.version] = this._data._transactionId[
						this.version
