		...
		<el-card class="main">
			<el-tabs v-model="version" type="card" @tab-click="ChangeVersion">
				...
				<el-tab-pane class="pane" label="区服管理v1.0" name="v1">
					...
					<el-container
						v-show="version === `v1`"
						style="width:100%; height:100%; min-width:1200px; margin:0 auto;"
					>
						<ApolloQuery
							:query="require('src/graphql/ZoneManagement.gql').games"
							:variables="{name:'game',version:versionNum}"
							@result="DoneOfGames"
						>
							<template slot-scope="{ result: { data } }">
								<el-aside
									width="16vw"
									style="min-width: 250px;background-color: rgb(238, 241, 246); height:100%"
								>
									<el-menu
										:default-active="defaultGame"
									...
									>
										<el-submenu index="1">
											...
											<el-menu-item
												...
												:key="item.id"
												:index="'1-' + (index + 1)"
												@click="selectGame($event,item)"
											>{{item.appName}}</el-menu-item>
										</el-submenu>
									</el-menu>
								</el-aside>
							</template>
						...
						<ApolloQuery
							v-if="appId"
							:query="require('src/graphql/ZoneManagement.gql').zones"
							:variables="{name:'zones',version: versionNum,data:{appId,pageCount:currentPage, refresh}}"
							@result="DoneOfZones"
						>
							<template slot-scope="{ result: { data} }">
								<el-main style="height:100%; width:82vw;min-width:750px;padding-top:0;">
									...
									<el-table
										...
										style="width: 100%"
									>
										<el-table-column prop="zoneId" label="游戏区ID"></el-table-column>
										<el-table-column prop="mainGameZoneId" label="主区ID"></el-table-column>
										<el-table-column prop="zoneName" label="游戏区名"></el-table-column>
										<el-table-column prop="gameZoneId" label="第三方游戏区ID"></el-table-column>
										<el-table-column prop="status" label="状态"></el-table-column>
										<el-table-column prop="openTime" label="开服时间"></el-table-column>

										<el-table-column width="120px" label="操作">
											<template slot-scope="scope">
												<span class="el-icon-edit-outline"></span>
												<el-button @click="handleEditer(scope.$index, scope.row)" type="text" size="small">编辑</el-button>
												<span class="el-icon-edit-outline"></span>
												<el-button @click="mergeZone(scope.$index, scope.row)" type="text" size="small">合区</el-button>
											</template>
										</el-table-column>
									</el-table>
									...
									<el-dialog title :visible.sync="dialogTableVisible" :append-to-body="true" center>
										<el-form :model="form1" :rules="rules1" :label-width="formLabelWidth">
											<el-form-item v-if="formItemVariable.zoneId" label="游戏区ID：" prop="zoneId">
												<el-input
													v-model="form1.zoneId"
													autocomplete="off"
													:placeholder="translation.placeholder.zoneId"
													disabled
												></el-input>
											</el-form-item>
											<el-form-item v-if="formItemVariable.zoneName" label="游戏区名称：" prop="zoneName">
												<el-input
													v-model="form1.zoneName"
													autocomplete="off"
													:placeholder="translation.placeholder.zoneName"
													:disabled="form1EditDISable.zoneName"
												></el-input>
											</el-form-item>
											<el-form-item
												v-if="formItemVariable.mainGameZoneId && !dialogTableVisible3"
												label="主区ID："
												prop="mainGameZoneId"
											>
												<el-input
													v-model="form1.mainGameZoneId"
													autocomplete="off"
													:placeholder="translation.placeholder.mainGameZoneId"
													:disabled="form1EditDISable.mainGameZoneId"
												></el-input>
											</el-form-item>
											<el-form-item v-if="formItemVariable.gameZoneId" label="第三方游戏区ID：" prop="gameZoneId">
												<el-input
													v-model="form1.gameZoneId"
													autocomplete="off"
													:placeholder="translation.placeholder.thirdGameZoneId"
													:disabled="form1EditDISable.gameZoneId"
												></el-input>
											</el-form-item>
											...
											<el-form-item
												v-if="formItemVariable.openTime"
												label="开服时间："
												class="zone-time"
												prop="openTime"
											>
											...
											<ApolloQuery
												:query="require('src/graphql/ZoneManagement.gql').mainZoneList"
												:variables="{name:'mainZoneList',version:'1',data:{appId}}"
												@result="DoneOfMainZoneList"
											>
												<template slot-scope="{ result: { data } }">
													<el-form-item
														v-if="formItemVariable.mainGameZoneId && dialogTableVisible3"
													...
													>
														<el-select
															v-model="form1.mainGameZoneId"
															:placeholder="translation.placeholder.mainGameZoneName"
														>
															<el-option
																v-for="(item,index) in mainZoneList"
																:key="item.id"
																:label="item.zoneName"
																:value="index"
															></el-option>
										...
										<ApolloMutation
											...
											:mutation="require('src/graphql/ZoneManagement.gql').addZoneInfo"
											@done="DoneOfAddZone"
										>
										...
										<ApolloMutation
											v-else-if="dialogTableVisible2"
											:mutation="require('src/graphql/ZoneManagement.gql').editZoneInfo"
											@done="DoneOfEditZone"
										>
											<template slot-scope="{ mutate }">
												<div slot="footer" class="dialog-footer">
										...
										<ApolloMutation
											v-else-if="dialogTableVisible3"
											:mutation="require('src/graphql/ZoneManagement.gql').mergeZone"
											@done="DoneOfMergeZone"
										>
											<template slot-scope="{ mutate }">
												<div slot="footer" class="dialog-footer">
													<el-row type="flex" justify="center">
				...
				<el-tab-pane class="pane" label="区服管理v2.0" name="v2">
					...
					>
						<ApolloQuery
							:query="require('src/graphql/ZoneManagement.gql').games"
							:variables="{name:'games',version:versionNum}"
							@result="DoneOfGames"
						>
							<template slot-scope="{ result: { data } }">
								<el-aside
							...
							<template slot-scope="{ result: { data} }">
								<el-main style="height:100%; width:80vw;min-width:750px;padding-top:0;">
									...
									<el-table
										...
										style="width: 100%"
									>
										<el-table-column prop="appId" width="80px" label="应用ID"></el-table-column>
										<el-table-column prop="gameZoneId" label="区服ID" width="50px"></el-table-column>
										<el-table-column prop="openTime" label="开服时间" width="200px"></el-table-column>
										<el-table-column prop="zoneName" label="区服名称"></el-table-column>
										<el-table-column prop="localName" width="60px" label="本地化区服名称"></el-table-column>
										<el-table-column prop="zoneDesc" label="区服描述"></el-table-column>
										<el-table-column prop="thirdGameZoneId" width="60px" label="游戏的区服ID"></el-table-column>
										<el-table-column prop="createTime" label="创建时间"></el-table-column>
										<el-table-column prop="modifyTime" label="修改时间"></el-table-column>
										<el-table-column prop="domain" width="60px" label="区服域名"></el-table-column>
										<el-table-column prop="mainGameZoneId" width="60px" label="主区服ID"></el-table-column>
										<el-table-column prop="status" width="100px" label="状态"></el-table-column>
										<el-table-column width="120px" label="操作">
											<template slot-scope="scope">
												<span class="el-icon-edit-outline"></span>
												<el-button @click="handleEditer(scope.$index, scope.row)" type="text" size="small">编辑</el-button>
									...
									>
										<el-form :model="form1" :rules="rules1" :label-width="formLabelWidth">
											<el-form-item v-if="formItemVariable.appId" label="游戏名称：" prop="appId">
												...
												<el-select
													v-model="form1.appId"
													:placeholder="translation.placeholder.appId"
													:disabled="form1EditDISable.appId"
												>
													<el-option
														v-for="item in games"
														:key="item.id"
											...
											<el-form-item v-if="formItemVariable.gameZoneId" label="区服ID：" prop="gameZoneId">
												<el-input
													v-model="form1.gameZoneId"
													autocomplete="off"
													:placeholder="translation.placeholder.gameZoneId"
													:disabled="form1EditDISable.gameZoneId"
												></el-input>
											</el-form-item>
											...
											<el-form-item v-if="formItemVariable.localName" label="本地化区服名称：" prop="localName">
												<el-input
													v-model="form1.localName"
													autocomplete="off"
													:placeholder="translation.placeholder.localName"
												></el-input>
											</el-form-item>
											<el-form-item v-if="formItemVariable.zoneDesc" label="区服描述：" prop="zoneDesc">
												<el-input
													v-model="form1.zoneDesc"
													autocomplete="off"
													:placeholder="translation.placeholder.zoneDesc"
												></el-input>
											</el-form-item>
											<el-form-item
												v-if="formItemVariable.thirdGameZoneId"
												label="游戏区服ID："
												prop="thirdGameZoneId"
											>
												<el-input
													v-model="form1.thirdGameZoneId"
													autocomplete="off"
													:placeholder="translation.placeholder.thirdGameZoneId"
												></el-input>
											</el-form-item>
											<el-form-item v-if="formItemVariable.domain" label="区服域名：">
												<el-input
													...
													autocomplete="off"
													:placeholder="translation.placeholder.domain"
												></el-input>
											</el-form-item>

											<ApolloQuery
												:query="require('src/graphql/ZoneManagement.gql').mainZoneList"
												:variables="{name:'mainZoneList',version:'2',data:{appId}}"
												@result="DoneOfMainZoneList"
											>
												<template slot-scope="{ result: { data } }">
													<el-form-item
														v-if="formItemVariable.mainGameZoneId"
														label="区服名称："
														:label-width="formLabelWidth"
														prop="mainGameZoneId"
													>
...
<script lang="ts">
	...
	export default class ZoneManagement extends Vue {
				...
				mutate({
					variables: {
						...
						data: {
							...
							gameZoneId: this.mainZoneList[this.form1.mainGameZoneId]
								.mainGameZoneId,
							zoneName: this.mainZoneList[this.form1.mainGameZoneId]
								.mainGameZoneName,
							...
							mainGameZoneId: this.mainZoneList[this.form1.mainGameZoneId]
								.mainGameZoneId
						}
					}
				});
		...
		mergeZone2(mutate) {
			if (this.form1.zoneId || this.form1.mainGameZoneId) {
				mutate({
					variables: {
						name:"mergeZone",
						version: "1",
